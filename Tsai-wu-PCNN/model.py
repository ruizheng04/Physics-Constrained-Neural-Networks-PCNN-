...
class TsaiWuPINN(nn.Module):
    ...
    def __init__(self, layer_sizes=[20, 512, 1], dropout_rate=0.01, activation='swiglu'):
        ...
        for i in range(len(layer_sizes) - 1):
            ...
            # Activation layer (only create for hidden layers, output layer forced to use Sigmoid)
        ...
        print(f"   Architecture: {' -> '.join(map(str, layer_sizes))} (optimal config)")
        print(f"   Input: 20D enhanced features (14 material properties + 6 stress directions)")
        print(f"   Output: 1D failure length L")
        print(f"   Hidden layer activation: {activation} (optimal config)")
        print(f"   Dropout rate: {dropout_rate} (optimal config)")
            ...
            if isinstance(m, nn.Linear):
                ...
                if is_output_layer:
                    ...
                    if self.activation == 'swiglu':
                        ...
                        with torch.no_grad():
                            # For layers with width 512, use smaller scaling factor
    ...
    def forward(self, x):
        """Forward propagation - outputs 1D failure length L, last layer forced to use Sigmoid for [0,1] output"""
        # Input check
        if x.shape[1] != 20:
        ...
        if x.min() < -0.1 or x.max() > 1.1:
            print(f"⚠️ Warning: Input data out of expected range [{x.min():.4f}, {x.max():.4f}]")
        
        # Forward propagation through hidden layers (except last layer)
        ...
        # Force use of Sigmoid to ensure output in [0,1] range, regardless of configured activation
